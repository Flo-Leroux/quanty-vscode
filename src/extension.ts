...
export async function activate(context: vscode.ExtensionContext) {
	...
	const langserverFolder = 'langserver';
	const executable = 'main';

    console.log(path.join(workspaceRoot, langserverFolder, executable))
    ...
    let serverPath = path.join(workspaceRoot, langserverFolder, executable)
	...
	let clientOptions: lspclient.LanguageClientOptions = {
		...
		documentSelector: [
            { scheme: 'file', language: 'quanty' },
		    // { scheme: 'file', language: 'yaml' }
        ],
		synchronize: {
            fileEvents: vscode.workspace.createFileSystemWatcher('**/.quanty.yaml')
		},
